...
\paragraph{Geometric units.} We use units with $G = c = 1$, so mass has dimensions of time (or length). The total mass $M = m_1 + m_2$ sets the characteristic timescale. Define the solar mass timescale:
\begin{equation}
    \tau_\odot \equiv \frac{G M_\odot}{c^3} \approx 4.926 \times 10^{-6} \, \text{s}
\end{equation}
For a binary with total mass $M$ (expressed in solar masses), the geometric timescale is $M \tau_\odot$. Physical frequency is recovered from dimensionless frequency $Mf$ via:
\begin{equation}
    f_{\rm phys} = \frac{Mf}{M \tau_\odot} = \frac{Mf}{M/M_\odot} \times \frac{1}{\tau_\odot} \approx \frac{Mf}{M/M_\odot} \times 2.03 \times 10^5 \, \text{Hz}
\end{equation}
...
The dependence on total mass deserves careful treatment. We \emph{define} a dimensionless reduced waveform $H_{\ell m}$ by factoring out the mass and distance dependence:
\begin{equation}
    \boxed{
    \tilde{h}_{\ell m}(f_{\rm phys}) \equiv \frac{(G M / c^3)^2}{D_L} \times H_{\ell m}(\Mf; \veclambda)
    }
    \label{eq:mass_scaling}
\end{equation}
where $\Mf = G M f_{\rm phys} / c^3$ is the dimensionless frequency and $H_{\ell m}$ depends only on $\Mf$ and intrinsic parameters $\veclambda$.

The prefactor $(GM/c^3)^2 / D_L$ has dimensions of time (consistent with $\tilde{h}$ being a Fourier transform). In geometric units ($G = c = 1$), this simplifies to $M^2 / D_L$.

\paragraph{Consistency check.} The Newtonian inspiral amplitude scales as $\tilde{A}_{22} \propto \mathcal{M}_c^{5/6} f^{-7/6} / D_L$ where $\mathcal{M}_c = M \eta^{3/5}$ is the chirp mass. Rewriting in terms of $\Mf$:
\begin{equation}
    f^{-7/6} = M^{7/6} (Mf)^{-7/6}, \quad \mathcal{M}_c^{5/6} = M^{5/6} \eta^{1/2}
\end{equation}
so $\tilde{A}_{22} \propto M^2 (Mf)^{-7/6} \eta^{1/2} / D_L$, confirming that the $M^2/D_L$ prefactor is absorbed leaving $H_{22} \propto (Mf)^{-7/6} \eta^{1/2}$.

\textbf{For emulation}: The network learns the dimensionless shape $H_{\ell m}(\Mf; \veclambda)$. At inference:
\begin{enumerate}
    \item Given physical parameters $(M, D_L, f_{\rm phys})$, compute $\Mf = G M f_{\rm phys} / c^3$
    \item Evaluate $H_{\ell m}(\Mf; \veclambda)$ from network
    \item Apply prefactor: $\tilde{h}_{\ell m} = (G M / c^3)^2 / D_L \times H_{\ell m}$
\end{enumerate}
...
The polarisations $h_+(t)$ and $h_\times(t)$ are real-valued functions. Although the complex strain $h = h_+ - i h_\times$ is itself complex, the reality of its components implies:
\begin{equation}
    \tilde{h}(-f) = \tilde{h}^*(f)
\end{equation}
We need only model positive frequencies $f > 0$; negative frequency content is determined by this conjugate relation. The standard matched-filter inner product uses only positive frequencies, with the ``$4 \, \text{Re}$'' prefactor accounting for the negative-frequency contribution.

\subsection{Conjugate Mode Symmetry (Non-Precessing)}
\label{sec:conjugate_symmetry}

For non-precessing systems with \emph{aligned} spins, the waveform has a reflection symmetry about the orbital plane. In the \emph{time domain}, this implies:
\begin{equation}
    h_{\ell,-m}(t) = (-1)^\ell \, h_{\ell m}^*(t)
    \label{eq:conjugate_time}
\end{equation}
Taking the Fourier transform:
\begin{equation}
    \tilde{h}_{\ell,-m}(f) = (-1)^\ell \, \tilde{h}_{\ell m}^*(-f)
    \label{eq:conjugate_freq}
\end{equation}
This relates the $m < 0$ mode at frequency $f$ to the $m > 0$ mode at frequency $-f$.

\textbf{Implication for emulation.}
For quasi-circular non-precessing binaries, the $m > 0$ and $m < 0$ modes are not independent---they are related by the symmetry \eqref{eq:conjugate_freq}. We therefore emulate only $\htlm(f)$ for $m > 0$ and $f > 0$.

\textbf{Strain reconstruction.}
Within the standard matched-filter inner product (one-sided, with the ``$4\,\text{Re}$'' prefactor), the strain at positive frequency is computed as:
\begin{equation}
    \boxed{
    \tilde{h}(f > 0) = \sum_{\ell} \sum_{m > 0} \htlm(f) \, \Ylm(\iota, \varphi_0)
    }
    \label{eq:strain_reconstruction}
\end{equation}
The negative-frequency contribution (and hence the $m < 0$ mode content) is accounted for by the inner product convention. When a two-sided Fourier representation is required (e.g., for IFFT to time domain), one first assembles $\tilde{h}(f > 0)$ using \eqref{eq:strain_reconstruction}, then constructs $\tilde{h}(f < 0) = \tilde{h}^*(-f)$ from the full-strain reality condition.

For IMRPhenomXHM, the relevant modes are: $(2,2)$, $(2,1)$, $(3,3)$, $(3,2)$, $(4,4)$.
...
\textbf{Pros}: Physical interpretation (related to time-to-merger) \\
\textbf{Cons}: Large dynamic range at low frequency ($d\Phi/d(Mf) \propto (Mf)^{-8/3}$ in inspiral), integration accumulates errors, boundary condition needed
...
\end{equation}
giving $N$ points from $\Mf_{\min}$ to $\Mf_{\max}$.
...
\begin{tikzpicture}[
    ...
    \node[box] (input) {$(\Mf, \eta, \chi_{1z}, \chi_{2z})$};
...
\begin{tikzpicture}[
    ...
    \node[box] (input) {$(\eta, \chi_{1z}, \chi_{2z})$};
    \node[box, below=of input] (nn) {MLP};
...
The key approximation is the \textbf{stationary phase assumption}: each frequency component $f$ corresponds to a single time $t(f)$ via the time-frequency relation. In practice, the mapping $t(f)$ is derived from the \emph{dominant $(2,2)$ mode} and then applied to all modes---an approximation that becomes less accurate for subdominant modes with different frequency content. The Euler angles at that time, $\alpha(t(f)), \beta(t(f)), \gamma(t(f))$, are then used to rotate the modes:
...
\begin{itemize}
    \item \textbf{Mode mixing}: The Wigner rotation redistributes power among modes with different $m'$---this is the \emph{intended} effect. The limitation is that the rotation is evaluated at a single $t(f)$ per frequency, whereas true precession creates sidebands at multiple frequencies.
    \item \textbf{Multiple timescales}: Strong precession violates stationary phase---a single frequency $f$ may correspond to multiple times $t$, or the precession timescale may be comparable to the orbital timescale.
    \item \textbf{Transitional precession}: Near $L \sim S$, the frame can flip rapidly, causing the Euler angles to vary on timescales faster than the $(2,2)$ mode frequency evolution.
\end{itemize}
...
\paragraph{Time and phase alignment.}
\textbf{Critical}: All training waveforms must be consistently aligned before extracting amplitude and phase. A time shift $t_0$ in the time domain corresponds to a phase term $e^{-2\pi i f t_0}$ in the frequency domain, adding a linear component $2\pi f t_0$ to the phase. If waveforms have different effective $t_0$ values (due to different merger times relative to the data window), the phase $\Phi(\Mf)$ will contain a parameter-dependent linear trend that is extremely difficult for the network to learn.

\textbf{Alignment procedure}:
\begin{enumerate}
    \item Generate waveforms with a common reference (e.g., coalescence at $t_c = 0$)
    \item Alternatively, post-process by fitting and removing a linear phase: $\Phi(f) \to \Phi(f) - 2\pi f t_0 - \phi_0$
    \item Fix the $(2,2)$ phase at the reference frequency: $\Phi_{22}(\Mf_{\rm ref}) = 0$
\end{enumerate}
Without proper alignment, the phase emulator will likely fail to converge.

\paragraph{Data format.}
Store in HDF5:
\begin{itemize}
    \item Frequency grid: $\Mf$ values (common to all waveforms)
    \item Per waveform: parameters $(\eta, \chi_{1z}, \chi_{2z})$, amplitudes, phases for each mode
    \item Metadata: LAL version, waveform approximant, alignment convention used
\end{itemize}
...
The maximisation over $t_c$ is typically performed via inverse FFT of the frequency-domain inner product integrand, which finds the optimal time shift efficiently. This operation is differentiable (FFTs are linear), enabling end-to-end training with mismatch loss if desired.
