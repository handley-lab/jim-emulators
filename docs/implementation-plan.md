...
This is the working implementation plan for **Jim Emulators**, a JAX-based waveform emulation library. This plan synthesizes the architectural decisions from **CosmoPower/Speculator**, the physics handling from **ripple**, and the specific requirements of the **jim** ecosystem.

> **Note**: Initially generated by Gemini from analysis of CosmoPower, ripple, and paper sources (~320K tokens), then refined based on team discussion.
...
*   **Waveform**: `IMRPhenomXPHM` (with higher modes).
    *   **Rationale**: Use XPHM directly rather than XAS. With aligned spins, XPHM reduces to XHM behavior automatically. This builds infrastructure once; we simplify by restricting parameter space, not changing the waveform model.
    *   Higher modes produce high-frequency "wiggles" important for waveform fidelity.
...
```python
def gen_IMRPhenomXPHM_emu(f, theta, f_ref):
    """
    f: Frequency grid (JAX array)
    theta: [Mc, eta, chi1, chi2, D_L, tc, phic, inclination]

    Note: With aligned spins (chi1, chi2 as scalars), reduces to XHM behavior.
    """
...
2.  [ ] **Data Gen**: Write `generate_data.py` (LAL XPHM → H5).
3.  [ ] **Components**: Implement `WaveformPCA` and `EmulatorNet` (w/ Speculator activation).
4.  [ ] **Training**: Train on XPHM with aligned spins (3D: η, χ₁z, χ₂z).
5.  [ ] **Validation**: Run mismatch tests against LAL.
6.  [ ] **Integration**: Write the `gen_IMRPhenomXPHM_emu` wrapper and test differentiability.
7.  [ ] **Extension**: Extend to full 7D precessing case.