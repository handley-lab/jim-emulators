...
\author{Will Handley \& James Alvey}
...
The development methodology employed a highly accelerated AI-assisted loop using three frontier LLMs:
\begin{itemize}
    \item \textbf{Claude 4.5 Sonnet} via Claude Code for primary development and code generation
    \item \textbf{Gemini 3} for iterative review and validation
    \item \textbf{GPT 5.2} for external code review and theory verification
\end{itemize}

\noindent The workflow involved:
\begin{enumerate}
    \item \textbf{Deep Research:} Autonomous literature surveys using LLMs to identify state-of-the-art architectures (identifying the CosmoPower/Speculator lineage).
    \item \textbf{Iterative Review:} Theoretical documents (\texttt{theory/*.tex}) were drafted and subjected to critical review by external LLMs to identify physical inconsistencies before implementation.
    \item \textbf{Rapid Prototyping:} Five distinct transcript sessions (\texttt{transcripts/*.txt}) captured the evolution from initial planning at 09:00 to final validation at 19:00, utilizing automated code generation and immediate feedback loops for debugging.
\end{enumerate}

\section{Physical Foundations and Infrastructure}

\subsection{LAL Infrastructure and Mode Decomposition}
A robust wrapper around \texttt{lalsimulation} was implemented (\texttt{src/jim\_emulators/waveforms/lal\_waveforms.py}) to generate reference waveforms. A crucial physical validation step verified that the IMRPhenomXPHM waveform is additive to machine precision:
\begin{equation}
...
Training data was generated (\texttt{scripts/generate\_data.py}) with the following specifications to ensure numerical stability:
\begin{itemize}
...
The emulator architecture (\texttt{src/jim\_emulators/emulator/}) follows the \textit{CosmoPower} and \textit{Speculator} paradigm: Principal Component Analysis (PCA) for compression followed by a Multi-Layer Perceptron (MLP).

\subsection{PCA Compression}
PCA was applied separately to the log-amplitude and unwrapped phase of the waveform (\texttt{src/jim\_emulators/emulator/pca.py}).
\begin{itemize}
...
The regression from parameters $\vec{\lambda} = \{\eta, \chi_{1z}, \chi_{2z}\}$ to PCA coefficients was performed using \texttt{Flax} (\texttt{src/jim\_emulators/emulator/network.py}):
\begin{itemize}
...
Initial training attempts using a single network for both amplitude and phase failed due to loss scaling imbalances. The successful strategy (\texttt{scripts/train.py}) employed \textbf{Dual Networks}: one dedicated MLP for amplitude coefficients and a separate MLP for phase coefficients. Training utilized the AdamW optimizer with gradient clipping and a cosine decay learning rate schedule. Best practices are documented in \texttt{docs/jax-flax-best-practices.md}.

\section{Validation Results}

The trained emulator for the dominant $(2,2)$ mode was validated (\texttt{scripts/validate\_emulator.py}) against fresh LAL waveforms not seen during training.
