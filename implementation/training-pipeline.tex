...
\State $[\mathbf{c}^{(A)}, \mathbf{c}^{(\Phi)}] \gets \text{MLP}(\hat{\eta}, \hat{\chi}_{1z}, \hat{\chi}_{2z})$
...
\begin{tikzpicture}[
    ...
    data/.style={rectangle, rounded corners, draw, fill=blue!10, minimum width=1.8cm, minimum height=0.6cm, align=center, font=\small},
    arrow/.style={-{Stealth[length=2mm]}, thick}
]
    % Input
    ...
    % PCA coeffs
    \node[data, right=of mlp] (coeffs) {$\mathbf{c}^{(A)}, \mathbf{c}^{(\Phi)}$};

    % PCA recon
    \node[box, right=of coeffs] (pca) {PCA\\reconstruct};

    % Denorm
    \node[box, right=of pca] (denorm) {Denormalize\\outputs};
    ...
    \draw[arrow] (mlp) -- (coeffs);
    \draw[arrow] (coeffs) -- (pca);
